...
export const blankItemSchema = z
  .object({
  ...
  .superRefine((item, ctx) => {
    ...
    if (blanks !== 1) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Each passage needs exactly one ___ blank",
        path: ["sentence"],
      });
    }
    const normalizedAnswer = normalizeBlankKey(item.answer);
    const decoyKeys = item.decoys.map(normalizeBlankKey);
    if (decoyKeys.includes(normalizedAnswer)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Decoys must not duplicate the answer",
        path: ["decoys"],
      });
    }
    if (new Set(decoyKeys).size !== decoyKeys.length) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Decoy choices must be unique",
        path: ["decoys"],
      });
    }
  });
...
export const sortGameSchema = z
  ...
  .superRefine((game, ctx) => {
    ...
    game.items.forEach((item, index) => {
      if (item.scoring === "auto") {
        if (!item.bucketId || !bucketIds.has(item.bucketId)) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Auto-scored items need a valid bucketId",
            path: ["items", index, "bucketId"],
          });
        }
      } else if (item.bucketId && !bucketIds.has(item.bucketId)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "bucketId must match a bucket",
          path: ["items", index, "bucketId"],
        });
      }
