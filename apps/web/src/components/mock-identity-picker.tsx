...
export function MockIdentityPicker() {
  ...
  // The server decides whether mock login exists at all; production never enables it.
  ...
  useEffect(() => {
    ...
    fetchAuthStatus()
      .then((status) => {
        if (!cancelled) setMockEnabled(status.mockEnabled);
      })
      .catch(() => {
  ...
  if (mockEnabled === null) {
    return <main className="jose-login">Checking sign-in mode…</main>;
  }
    ...
    return (
      <main className="jose-login">
        ...
        <section className="jose-login__card">
          ...
          <a className="jose-login__primary" href="/login">
            Back to sign-in
          </a>
        </section>
      </main>
