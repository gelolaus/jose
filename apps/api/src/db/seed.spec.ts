...
describe("explicit versioned seeding", () => {
  ...
  it("preserves deletion of a seeded extra and Ateneo after restart + reseed", async () => {
    ...
    try {
      ...
      const beforeAteneo = await opened.db
        .select({ id: modules.id })
        .from(modules)
        .where(eq(modules.id, "ateneo-days"));
      expect(beforeAteneo).toHaveLength(1);

      // Editorial deletes (same cascade cleanup the teach API uses for levels/modules).
      await opened.db
      ...
      // Simulate process restart: schema ensure + optional seed command.
