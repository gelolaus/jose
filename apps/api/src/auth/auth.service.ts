...
export class AuthService {
  ...
  constructor(
    ...
    try {
      ...
      if (error instanceof AuthConfigError) {
        // Keep process bootable; auth routes report the configuration problem.
        this.config = loadAuthConfig({ ...process.env, JOSE_AUTH_MODE: "disabled" });
  ...
  async me(token: string | undefined | null): Promise<{ authenticated: boolean; user: SessionUser | null }> {
