...
export class CurriculumService {
  ...
  async getPlayLevel(levelId: string): Promise<PlayLevelResponse> {
    ...
    if (ctx.level.kind === "lesson") {
      ...
      payload.chest = await this.readChestContent(levelId, ctx.level.title);
    }
    return payload;
  }
  ...
  async completeLevel(levelId: string) {
    ...
    if (ctx.level.kind === "chest") {
      artifactAwarded = await this.awardChestArtifact(levelId, ctx.level.title);
    }
    const learner = await this.getLearner();
    return { completed: true, firstTime: first, learner, artifactAwarded };
  }

  async listArtifacts(): Promise<ArtifactsResponse> {
    const rows = await this.db
      .select()
      .from(learnerArtifacts)
      .where(eq(learnerArtifacts.learnerId, DEMO_LEARNER_ID))
  ...
  /** Returns true only when a new artifact row is inserted. */
  private async awardChestArtifact(
    ...
    const [existing] = await this.db
      ...
      .where(
        and(
          eq(learnerArtifacts.learnerId, DEMO_LEARNER_ID),
