...
describe("CurriculumService", () => {
  ...
  it("lists published modules including the featured Rizal path", async () => {
    const body = modulesResponseSchema.parse(
      await service.listPublishedModules(learnerId),
    );
  ...
  it("returns a schema-valid featured path", async () => {
    const path = pathResponseSchema.parse(await service.getFeaturedPath(learnerId));
  ...
  it("unlocks the next Ateneo days level only after the first is finished", async () => {
    await expect(service.getPlayLevel("ateneo-quiz", learnerId)).rejects.toThrow(
      /previous level/i,
    );
    await service.completeLevel("ateneo-welcome", learnerId);
    const play = await service.getPlayLevel("ateneo-quiz", learnerId);
  ...
  it("spends a heart on a miss and refills after a lesson", async () => {
    ...
    const before = await service.getLearner(learnerId);
    await service.recordMiss("ateneo-quiz", learnerId);
    const afterMiss = await service.getLearner(learnerId);
    ...
    await database.db
      ...
      .where(eq(learners.id, learnerId));
    await service.completeLevel("edu-binan", learnerId);
    expect((await service.getLearner(learnerId)).hearts).toBe(5);
  });
  ...
  it("blocks starting a game at zero hearts", async () => {
    ...
    await database.db
      ...
      .where(eq(learners.id, learnerId));
    try {
      await service.getPlayLevel("ateneo-quiz", learnerId);
