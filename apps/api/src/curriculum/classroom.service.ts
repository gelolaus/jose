...
export class ClassroomService {
  ...
  async listClasses(user: AuthUser): Promise<ClassSummary[]> {
  ...
  async createClass(user: AuthUser, body: unknown) {
    this.requireTeacher(user);
  ...
  async rotateInvite(user: AuthUser, classId: string) {
  ...
  async archiveClass(user: AuthUser, classId: string) {
  ...
  async joinClass(user: AuthUser, body: unknown) {
  ...
  async createAssignment(user: AuthUser, classId: string, body: unknown) {
  ...
  async listStudentAssignments(user: AuthUser): Promise<StudentAssignment[]> {
  ...
  async classReport(user: AuthUser, classId: string, assignmentId: string): Promise<ClassReport> {
    await this.requireOwnedClass(user, classId);
  ...
  async exportClassReportCsv(user: AuthUser, classId: string, assignmentId: string) {
    ...
    const attemptRows = await this.db
      ...
      .where(
        and(
          ...
          eq(attempts.contentRevisionId, assignment.contentRevisionId),
        ),
      );
  ...
  private async misconceptions(assignment: typeof assignments.$inferSelect) {
    const rows = await this.db
      .select()
      .from(attempts)
      .where(eq(attempts.contentRevisionId, assignment.contentRevisionId));
  ...
  private async requireOwnedClass(user: AuthUser, classId: string) {
    this.requireTeacher(user);
  ...
  private async ensureLearner(user: AuthUser) {
