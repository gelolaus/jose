...
| `mock` | Local/dev/CI without Entra. Signed mock OIDC boundary. **Rejected at boot in production.** |
...
For local mock development, use the repository-root `.env` (`JOSE_AUTH_MODE=mock`).
...
- APC email comes from validated ID token claims only after the OIDC library verifies state, nonce, signature, issuer, audience, and expiry.
...
- Missing, malformed, or ambiguous claims → deny. Jose never accepts a manually typed email as proof of Microsoft identity.
- Present APC claims create the Jose account and session directly; no mail provider or OTP is involved.
...
```bash
# Both apps load this single repository-root file.
cp .env.example .env
